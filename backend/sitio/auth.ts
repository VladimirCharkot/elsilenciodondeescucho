...
const is_admin: Middleware = async (req, res, next) => {
  if(req.user) { next(); } else { console.log(`Blocking non-admin request to ${req.path}`); res.status(401).json({ok: false}); }
}

const check_admin: Middleware = async (req, res) => {
  if(req.user) { res.status(200).json({ok: true}) } else { res.status(401).json({ok: false}); }
} 
...
export {passport, login, logout, is_admin, check_admin}
