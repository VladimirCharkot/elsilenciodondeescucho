...
exports.escritos = async (req, res) => {
  ...
  res.render('indice', { titulo: 'Índice', cont: indice_html })
}

/* Página con un escrito */
exports.escrito = async (req, res) => {
  let texto_html = await traer_texto(req.params.eid)
  if (!texto_html) return res.status(404).json({status: 404, mensaje: 'No existe'})
  res.render('escrito', { titulo: 'Escritos', cont: texto_html })
}
...
/* Plancha el md generado a partir de leer public/textos al índice de la página */
let indice_fs_a_md = async () => {
  // construir_indice().then(planchar_indice_a_md).then(fs.writeFile)
  await fs.writeFile('views/indice_default.md', await indice_a_md(await construir_indice()))
}
